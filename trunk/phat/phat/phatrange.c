...
static void phat_range_finalize       (GObject          *object);
static void phat_range_size_request   (GtkWidget        *widget,
...
static gboolean phat_range_expose         (GtkWidget        *widget,
                                      GdkEventExpose   *event);
static gboolean phat_range_button_press   (GtkWidget        *widget,
                                      GdkEventButton   *event);
static gboolean phat_range_button_release (GtkWidget        *widget,
                                      GdkEventButton   *event);
static gboolean phat_range_motion_notify  (GtkWidget        *widget,
                                      GdkEventMotion   *event);
static gboolean phat_range_enter_notify   (GtkWidget        *widget,
                                      GdkEventCrossing *event);
static gboolean phat_range_leave_notify   (GtkWidget        *widget,
                                      GdkEventCrossing *event);
static gboolean phat_range_grab_broken (GtkWidget          *widget,
				       GdkEventGrabBroken *event);
static void phat_range_grab_notify    (GtkWidget          *widget,
				      gboolean            was_grabbed);
static void phat_range_state_changed  (GtkWidget          *widget,
				      GtkStateType        previous_state);
static gint phat_range_scroll_event   (GtkWidget        *widget,
                                      GdkEventScroll   *event);
static void phat_range_style_set      (GtkWidget        *widget,
                                      GtkStyle         *previous_style);
static void update_slider_position   (PhatRange	       *range,
				      gint              mouse_x,
				      gint              mouse_y);
static void stop_scrolling           (PhatRange         *range);

/* Range methods */

static void phat_range_move_slider              (PhatRange         *range,
                                                GtkScrollType     scroll);

/* Internals */
static void          phat_range_scroll                   (PhatRange      *range,
                                                         GtkScrollType  scroll);
static gboolean      phat_range_update_mouse_location    (PhatRange      *range);
static void          phat_range_calc_layout              (PhatRange      *range,
							 gdouble	adjustment_value);
static void          phat_range_get_props                (PhatRange      *range,
                                                         gint          *slider_width,
                                                         gint          *stepper_size,
                                                         gint          *focus_width,
                                                         gint          *trough_border,
...
static void          phat_range_calc_request             (PhatRange      *range,
                                                         gint           slider_width,
                                                         gint           stepper_size,
                                                         gint           focus_width,
                                                         gint           trough_border,
...
static void          phat_range_adjustment_changed       (GtkAdjustment *adjustment,
                                                         gpointer       data);
static void          phat_range_add_step_timer           (PhatRange      *range,
                                                         GtkScrollType  step);
static void          phat_range_remove_step_timer        (PhatRange      *range);
static void          phat_range_reset_update_timer       (PhatRange      *range);
static void          phat_range_remove_update_timer      (PhatRange      *range);
static GdkRectangle* get_area                           (PhatRange      *range,
                                                         MouseLocation  location);
static gboolean      phat_range_real_change_value        (PhatRange      *range,
                                                         GtkScrollType  scroll,
                                                         gdouble        value);
static void          phat_range_update_value             (PhatRange      *range);
static gboolean      phat_range_key_press                (GtkWidget     *range,
							 GdkEventKey   *event);
...
phat_range_class_init (PhatRangeClass *class)
{
  ...
  gobject_class->finalize = phat_range_finalize;
  object_class->destroy = phat_range_destroy;

  widget_class->size_request = phat_range_size_request;
  ...
  widget_class->expose_event = phat_range_expose;
  widget_class->button_press_event = phat_range_button_press;
  widget_class->button_release_event = phat_range_button_release;
  widget_class->motion_notify_event = phat_range_motion_notify;
  widget_class->scroll_event = phat_range_scroll_event;
  widget_class->enter_notify_event = phat_range_enter_notify;
  widget_class->leave_notify_event = phat_range_leave_notify;
  widget_class->grab_broken_event = phat_range_grab_broken;
  widget_class->grab_notify = phat_range_grab_notify;
  widget_class->state_changed = phat_range_state_changed;
  widget_class->style_set = phat_range_style_set;
  widget_class->key_press_event = phat_range_key_press;

  class->move_slider = phat_range_move_slider;
  class->change_value = phat_range_real_change_value;
  ...
  signals[VALUE_CHANGED] =
    g_signal_new (I_("value_changed"),
                  ...
                  NULL, NULL,
                  g_cclosure_marshal_VOID__VOID, /* correct? was _gtk_marshal_NONE__NONE  */
                  G_TYPE_NONE, 0);
  ...
  signals[ADJUST_BOUNDS] =
    g_signal_new (I_("adjust_bounds"),
                  G_TYPE_FROM_CLASS (gobject_class),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (PhatRangeClass, adjust_bounds),
                  NULL, NULL,
  ...
  signals[MOVE_SLIDER] =
    g_signal_new (I_("move_slider"),
                  ...
                  G_STRUCT_OFFSET (PhatRangeClass, move_slider),
                  NULL, NULL,
  ...
  /**
   * PhatRange:trough-under-steppers:
   *
			...
			GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_UPDATE_POLICY:
      phat_range_set_update_policy (range, g_value_get_enum (value));
      break;
    case PROP_ADJUSTMENT:
      phat_range_set_adjustment (range, g_value_get_object (value));
      break;
    case PROP_INVERTED:
      phat_range_set_inverted (range, g_value_get_boolean (value));
      break;
    case PROP_LOWER_STEPPER_SENSITIVITY:
      phat_range_set_lower_stepper_sensitivity (range, g_value_get_enum (value));
      break;
    case PROP_UPPER_STEPPER_SENSITIVITY:
      phat_range_set_upper_stepper_sensitivity (range, g_value_get_enum (value));
      break;
    default:
			...
			GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_INVERTED:
      ...
      break;
    case PROP_LOWER_STEPPER_SENSITIVITY:
      g_value_set_enum (value, phat_range_get_lower_stepper_sensitivity (range));
      break;
    case PROP_UPPER_STEPPER_SENSITIVITY:
      g_value_set_enum (value, phat_range_get_upper_stepper_sensitivity (range));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
phat_range_init (PhatRange *range)
{
  ...
  range->layout = g_new0 (PhatRangeLayout, 1);
 ...
 **/
void
phat_range_set_update_policy (PhatRange      *range,
...
/**
 * phat_range_get_update_policy:
 * @range: a #PhatRange
 *
 * Gets the update policy of @range. See phat_range_set_update_policy().
 *
 * Return value: the current update policy
 **/
GtkUpdateType
phat_range_get_update_policy (PhatRange *range)
{
  g_return_val_if_fail (PHAT_IS_RANGE (range), GTK_UPDATE_CONTINUOUS);
 ...
 **/
void
phat_range_set_inverted (PhatRange *range,
                        gboolean  setting)
{
  g_return_if_fail (PHAT_IS_RANGE (range));
...
/**
 * phat_range_get_inverted:
 * @range: a #PhatRange
 * 
 * Gets the value set by phat_range_set_inverted().
 * 
 * Return value: %TRUE if the range is inverted
 **/
gboolean
phat_range_get_inverted (PhatRange *range)
{
  g_return_val_if_fail (PHAT_IS_RANGE (range), FALSE);

  return range->inverted;
}

/**
 * phat_range_set_lower_stepper_sensitivity:
 * @range:       a #PhatRange
 * @sensitivity: the lower stepper's sensitivity policy.
 *
 * Sets the sensitivity policy for the stepper that points to the
 * 'lower' end of the PhatRange's adjustment.
 *
 * Since: 2.10
 **/
void
phat_range_set_lower_stepper_sensitivity (PhatRange           *range,
  ...
  if (range->layout->lower_sensitivity != sensitivity)
    {
      ...
      range->need_recalc = TRUE;
      phat_range_calc_layout (range, range->adjustment->value);
...
/**
 * phat_range_get_lower_stepper_sensitivity:
 * @range: a #PhatRange
 *
 ...
 **/
GtkSensitivityType
phat_range_get_lower_stepper_sensitivity (PhatRange *range)
{
  g_return_val_if_fail (PHAT_IS_RANGE (range), GTK_SENSITIVITY_AUTO);

  return range->layout->lower_sensitivity;
}

/**
 * phat_range_set_upper_stepper_sensitivity:
 * @range:       a #PhatRange
 * @sensitivity: the upper stepper's sensitivity policy.
 *
 * Sets the sensitivity policy for the stepper that points to the
 * 'upper' end of the PhatRange's adjustment.
 *
 * Since: 2.10
 **/
void
phat_range_set_upper_stepper_sensitivity (PhatRange           *range,
                                         GtkSensitivityType  sensitivity)
{
  g_return_if_fail (PHAT_IS_RANGE (range));
...
/**
 * phat_range_get_upper_stepper_sensitivity:
 * @range: a #PhatRange
 *
 * Gets the sensitivity policy for the stepper that points to the
 * 'upper' end of the PhatRange's adjustment.
 ...
 **/
GtkSensitivityType
phat_range_get_upper_stepper_sensitivity (PhatRange *range)
{
  g_return_val_if_fail (PHAT_IS_RANGE (range), GTK_SENSITIVITY_AUTO);
...
static void
phat_range_finalize (GObject *object)
{
  PhatRange *range = PHAT_RANGE (object);

  g_free (range->layout);

  (* G_OBJECT_CLASS (phat_range_parent_class)->finalize) (object);
}
...
phat_range_destroy (GtkObject *object)
{
  ...
  phat_range_remove_step_timer (range);
  phat_range_remove_update_timer (range);
...
static void
phat_range_size_request (GtkWidget      *widget,
                        GtkRequisition *requisition)
{
  PhatRange *range;
              ...
              GdkRectangle *area)
{
  ...
  gtk_paint_box (widget->style,
		 ...
		 &intersection, widget,
		 PHAT_RANGE_GET_CLASS (range)->stepper_detail,
  ...
  if (clicked && arrow_sensitive)
    {
      ...
      phat_range_get_props (PHAT_RANGE (widget),
...
static gboolean
phat_range_expose (GtkWidget      *widget,
		  GdkEventExpose *event)
{
  PhatRange *range;
  gboolean sensitive;
  ...
  if (gdk_rectangle_intersect (&expose_area,
                               ...
                               &area))
    {
      ...
      gtk_paint_slider (widget->style,
                        ...
                        &area,
                        widget,
                        PHAT_RANGE_GET_CLASS (range)->slider_detail,
...
static void
range_grab_add (PhatRange      *range,
                MouseLocation  location,
                gint           button)
{
  ...
  if (phat_range_update_mouse_location (range))
    gtk_widget_queue_draw (GTK_WIDGET (range));
}

static void
range_grab_remove (PhatRange *range)
{
  MouseLocation location;
...
range_get_scroll_for_grab (PhatRange      *range)
{ 
  gboolean invert;
...
static gdouble
coord_to_value (PhatRange *range,
                gint      coord)
{
  gdouble frac;
  gdouble value;
  ...
  phat_range_get_props (range, NULL, NULL, NULL, &trough_border, NULL,
...
static gboolean
phat_range_key_press (GtkWidget   *widget,
		     GdkEventKey *event)
{
  PhatRange *range = PHAT_RANGE (widget);
  ...
  return GTK_WIDGET_CLASS (phat_range_parent_class)->key_press_event (widget, event);
}

static gint
phat_range_button_press (GtkWidget      *widget,
			GdkEventButton *event)
{
  PhatRange *range = PHAT_RANGE (widget);
  ...
  if (range->layout->mouse_location == MOUSE_TROUGH  &&
      event->button == 1)
    {
      ...
      phat_range_add_step_timer (range, scroll);
           ...
           range->layout->mouse_location == MOUSE_SLIDER)
    {
	  ...
	  /* recalc slider, so we can set slide_initial_slider_position
           ...
           */
	  range->need_recalc = TRUE;
          phat_range_calc_layout (range, new_value);
...
static void
update_slider_position (PhatRange *range,
                        gint      mouse_x,
                        gint      mouse_y)
{
  gint delta;
...
static void 
stop_scrolling (PhatRange *range)
{
  range_grab_remove (range);
  phat_range_remove_step_timer (range);
  /* Flush any pending discontinuous/delayed updates */
  phat_range_update_value (range);
}

static gboolean
phat_range_grab_broken (GtkWidget          *widget,
		       GdkEventGrabBroken *event)
{
  PhatRange *range = PHAT_RANGE (widget);
...
static gboolean
phat_range_button_release (GtkWidget      *widget,
			  GdkEventButton *event)
{
  PhatRange *range = PHAT_RANGE (widget);
 ...
 **/
gdouble
_phat_range_get_wheel_delta (PhatRange           *range,
...
static gint
phat_range_scroll_event (GtkWidget      *widget,
			GdkEventScroll *event)
{
  PhatRange *range = PHAT_RANGE (widget);
  ...
  if (GTK_WIDGET_REALIZED (range))
    {
      GtkAdjustment *adj = PHAT_RANGE (range)->adjustment;
      gdouble delta;
      gboolean handled;

      delta = _phat_range_get_wheel_delta (range, event->direction);
...
static gboolean
phat_range_motion_notify (GtkWidget      *widget,
			 GdkEventMotion *event)
{
  PhatRange *range;
  gint x, y;
...
static gboolean
phat_range_enter_notify (GtkWidget        *widget,
			GdkEventCrossing *event)
{
  PhatRange *range = PHAT_RANGE (widget);
...
static gboolean
phat_range_leave_notify (GtkWidget        *widget,
			GdkEventCrossing *event)
{
  PhatRange *range = PHAT_RANGE (widget);
...
static void
phat_range_grab_notify (GtkWidget *widget,
		       gboolean   was_grabbed)
{
  if (!was_grabbed)
    stop_scrolling (PHAT_RANGE (widget));
}

static void
phat_range_state_changed (GtkWidget    *widget,
			 GtkStateType  previous_state)
{
...
static gboolean
layout_changed (PhatRangeLayout *layout1, 
		PhatRangeLayout *layout2)
{
			      ...
			      gpointer       data)
{
  ...
  PhatRangeLayout layout = *range->layout;
   ...
   * is therefore defined to be a filter on what the PhatRange
   * can input into the adjustment, not a filter that the PhatRange
...
static void
phat_range_style_set (GtkWidget *widget,
                     GtkStyle  *previous_style)
{
  PhatRange *range = PHAT_RANGE (widget);
  ...
  (* GTK_WIDGET_CLASS (phat_range_parent_class)->style_set) (widget, previous_style);
}

static void
step_back (PhatRange *range)
{
  gdouble newval;
  gboolean handled;
...
static void
phat_range_scroll (PhatRange     *range,
                  GtkScrollType scroll)
{
  switch (scroll)
    {
...
static void
phat_range_move_slider (PhatRange     *range,
                       GtkScrollType scroll)
{
  phat_range_scroll (range, scroll);
...
static void
phat_range_get_props (PhatRange  *range,
                     gint      *slider_width,
                     gint      *stepper_size,
                     gint      *focus_width,
                     gint      *trough_border,
...
static gboolean
phat_range_update_mouse_location (PhatRange *range)
{
  gint x, y;
  MouseLocation old;
  GtkWidget *widget;
...
static void
phat_range_calc_request (PhatRange      *range,
                        gint           slider_width,
                        gint           stepper_size,
                        gint           focus_width,
                        gint           trough_border,
                        ...
                        gint          *slider_length_p)
{
  ...
  if (PHAT_RANGE_GET_CLASS (range)->get_range_border)
    (* PHAT_RANGE_GET_CLASS (range)->get_range_border) (range, border);
...
static void
phat_range_calc_layout (PhatRange *range,
		       gdouble   adjustment_value)
{
  gint slider_width, stepper_size, focus_width, trough_border, stepper_spacing;
  gint slider_length;
  ...
  phat_range_update_mouse_location (range);
...
static gboolean
phat_range_real_change_value (PhatRange     *range,
                             GtkScrollType scroll,
                             gdouble       value)
{
  ...
  if (range->adjustment->value != value)
    {
      ...
      switch (range->update_policy)
        ...
        case GTK_UPDATE_DELAYED:
          phat_range_reset_update_timer (range);
          /* FALL THRU */
...
static void
phat_range_update_value (PhatRange *range)
{
  phat_range_remove_update_timer (range);
...
second_timeout (gpointer data)
{
  ...
  GDK_THREADS_ENTER ();
  range = PHAT_RANGE (data);
  phat_range_scroll (range, range->timer->step);
  GDK_THREADS_LEAVE ();
...
static void
phat_range_add_step_timer (PhatRange      *range,
                          GtkScrollType  step)
{
  GtkSettings *settings;
  guint        timeout;
  ...
  range->timer = g_new (PhatRangeStepTimer, 1);
...
static void
phat_range_remove_step_timer (PhatRange *range)
{
  if (range->timer)
    {
...
static void
phat_range_reset_update_timer (PhatRange *range)
{
  phat_range_remove_update_timer (range);
...
static void
phat_range_remove_update_timer (PhatRange *range)
{
